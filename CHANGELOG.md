...
* **Assignee initials removed from story table.** The initials prefix added in 2.27.0 (e.g. `  HR  Hendrickson, Robert [US] (MS)`) caused the assignee filter to break — the filter compared against the full cell text including the prefix, so typing a name would find no matches. The initials prefix has been removed and the assignee column reverts to displaying the name only. The full name tooltip on hover is retained.
