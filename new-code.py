def get_project_members(self, project_key: str):
    all_users = {}  # keyed by username to deduplicate

    # First get project-specific assignable users
    encoded = urllib.parse.quote(project_key)
    start = 0
    max_results = 200
    while True:
        url = (f"{self.base_url}/rest/api/{self.api_version}/"
               f"user/assignable/search?project={encoded}"
               f"&maxResults={max_results}&startAt={start}")
        req = urllib.request.Request(url, headers=self.headers)
        try:
            with urllib.request.urlopen(req, timeout=15) as resp:
                batch = json.loads(resp.read().decode())
                if not isinstance(batch, list) or not batch:
                    break
                for u in batch:
                    key = u.get("name") or u.get("accountId")
                    if key:
                        all_users[key] = u
                if len(batch) < max_results:
                    break
                start += max_results
        except Exception:
            break

    # Then get all instance users via wildcard search
    start = 0
    while True:
        url = (f"{self.base_url}/rest/api/{self.api_version}/user/search"
               f"?username=.&maxResults={max_results}&startAt={start}")
        req = urllib.request.Request(url, headers=self.headers)
        try:
            with urllib.request.urlopen(req, timeout=15) as resp:
                batch = json.loads(resp.read().decode())
                if not isinstance(batch, list) or not batch:
                    break
                for u in batch:
                    key = u.get("name") or u.get("accountId")
                    if key:
                        all_users[key] = u
                if len(batch) < max_results:
                    break
                start += max_results
        except Exception:
            break

    return list(all_users.values())
